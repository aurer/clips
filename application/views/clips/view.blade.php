...
@section('primary')
	...
	<ul id="taglist" class="reset tall">
		<li>Tags: &nbsp;</li>
		@foreach($tags as $tag)
			<li><a class="rounded {{ $tag->category }}" href="/tags/{{ $tag->slug }}">{{ $tag->tag }}</a></li>
		@endforeach
	</ul>
	
	<div class="actions right">
		<a class="btn" id="add-clip" href="/clips/{{ $clip->slug }}/edit/">Edit this clip</a>
		<a class="btn" id="delete-clip" href="#">Delete this clip</a>
		<a class="btn" id="view-raw-clip" href="/clips/{{ $clip->slug }}/raw/">View raw clip</a>
	</div>
	<p class="post-date">Posted {{ Common::time_since( $clip->created_at ) }}.</p>
	<div id="clips">
		<ol class="cliplist reset">
			<li class="clip" id="clip-{{ $clip->id }}">
				<pre class="clip-code visible"><code class="prettyprint">{{ HTML::entities($clip->code) }}</code></pre>
			</li>
