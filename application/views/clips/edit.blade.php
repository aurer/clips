...
@section('primary')
	...
	{{ Form::open(null, 'POST', array('id'=>'edit-form', 'class'=>'standard fullwidth') ) }}
		<fieldset class="main">
			<div class="field required field-title">
				{{ Form::label('title', 'Title') }}
				<span class="input">{{ Form::text('title', Input::old('title', $clip->title), array('class'=>'required', 'autofocus')) }}</span>
				{{ $errors->first('title', '<p>:message</p>'); }}
			</div>
			
			<div class="field required field-tags">
				{{ Form::label('tags','Tags') }}
				<span class="input">{{ Form::text('tags', Input::old('tags', $clip->tags), array('class'=>'required')) }}</span>
				{{ $errors->first('tags', '<p>:message</p>'); }}
			</div>
			
			<div class="field required field-code">
				{{ Form::label('code','Code') }}
				<span class="input">{{ Form::textarea('code', Input::old('code', $clip->code), array('rows'=>20, 'class'=>'required')) }}</span>
				{{ $errors->first('code', '<p>:message</p>'); }}
			</div>
			
			<div class="field field-private type-checkbox">
				<span class="input">{{ Form::checkbox('private', 'true', !empty($clip->private)) }}</span>
			...
			<div class="field field-description">
				...
				<span class="input">{{ Form::textarea('description', Input::old('description', $clip->description), array('rows'=>20)) }}</span>
				{{ $errors->first('description', '<p>:message</p>'); }}
			</div>
			
			<div class="field submit feild-submit">
				<span class="input">
					<a href="/clips/{{ $clip->slug }}" class="cancel btn">Cancel</a>
