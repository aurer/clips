cf = {
		...
		$('#clips').find('.clip-title').live('click', function(e){
			...
			if(cf.viewMode === 1 && cf.showCount < 2){
				if(cf.keyCode !== 18 ){
					...
					code.parent('li').siblings().find('.clip-options')[cf.hideAction](cf.showAllSpeed);
				}
			}
	...
	handleModalForms : function(){
		$('#new-clip').click(function(e){
    		e.preventDefault();
    		cf.loadForm($(this).attr('href') + ' #add-form');
	    });
	    ...
	    $('#clips').find('a.edit-clip').click(function(e){
	    	e.preventDefault();
	    	cf.loadForm($(this).attr('href')+' #edit-form');
	    });	
